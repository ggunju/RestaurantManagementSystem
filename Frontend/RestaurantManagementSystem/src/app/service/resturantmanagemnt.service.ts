...
export class ResturantmanagemntService {
  ...
  private rorderurl = "http://localhost:8080/payments/createTransaction";
  private cashstatusurl = "http://localhost:8080/restaurant/orders/updatestatus";
  private statusURL = "http://localhost:8080/restaurant/orders/status";
  private cashpayURL = "http://localhost:8080/payments/cash";
  ...
  //Create Razor Pay PAymnet
  public createTransaction(amount: any) {
    const url = `${this.rorderurl}/${amount}`;
    return this.http.get(url)
  }

  //change status to cash payment
  changeCashStatus(orderId: any): Observable<any> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    const url = `${this.cashstatusurl}/${orderId}`;

    return this.http.post(url, {}, { headers });
  }

  //change status to cash payment
  CashPay(orderId: any): Observable<any> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    const url = `${this.cashpayURL}/${orderId}`;
