...
export class PaymentOptionsComponent implements OnInit {
  ...
  redirectToPayment(paymentMethod: string) {
    if (paymentMethod === 'cod') {
      alert("We shall commence the preparation of your order after Cash Payment ");
      this.changeOrderStatusToCashPayment();
      this.router.navigateByUrl('/customer/myorders');
  ...
  changeOrderStatusToCashPayment() {
    this.orderId = parseInt(localStorage.getItem("generatedOrderId"));
    if (this.orderId) {
      this.resturantmanagemntservice.changeCashStatus(this.orderId).subscribe(
        () => {
          console.log("Order status changed to Cash Payment");
          localStorage.clear()
        },
        (error) => {
          console.error("Error changing order status to Cash Payment", error);
        }
      );
    }
  }
